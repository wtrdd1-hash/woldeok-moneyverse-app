...
- 구현: 179개 공식 모바일 endpoint를 번들 capability registry로 동기화하고 기존 전용 화면과 전체 기능센터를 결합한다. 관리자 11개 endpoint는 역할 재검증 후에만 노출한다.
- 테스트: Debian 13 로컬 빌드는 Android SDK 미설치로 BLOCKED. GitHub Android CI에서 SDK 36을 설치해 compile/test/assemble/bundle을 검증한다.
